...
def fusion_loss(features, prototypes, labels, gamma=1.0):
    num_views = len(features)
    batch_size = features[0].shape[0]
    num_classes = prototypes[0].shape[0]
    logits_all_views = [] 
    preds_all_views = [] 
    for v in range(num_views):
        h = features[v]
        p = prototypes[v]
        logits = torch.matmul(h, p.T)
        logits_all_views.append(logits)
        preds = torch.argmax(logits, dim=1) 
        preds_all_views.append(preds)
    preds_all_views_tensor = torch.stack(preds_all_views, dim=0).transpose(0, 1)
    final_preds = []
    for i in range(batch_size):
        votes = preds_all_views_tensor[i] 
        majority_vote = torch.mode(votes).values.item()
        final_preds.append(majority_vote)
    final_preds = torch.tensor(final_preds, device=labels.device)
    intra_view_loss = 0.0
    for logits in logits_all_views:
        intra_view_loss += F.cross_entropy(logits, labels)
    intra_view_loss = intra_view_loss / num_views 
    inter_view_loss = 0.0
    for i in range(num_views):
        for j in range(i + 1, num_views):
            inter_view_loss += (preds_all_views[i] != preds_all_views[j]).float().sum()
    inter_view_loss = inter_view_loss / (batch_size * (num_views * (num_views - 1) / 2))
    total_loss = intra_view_loss + gamma * inter_view_loss
    return total_loss, intra_view_loss, inter_view_loss, final_preds


def compute_saprmvl_loss(num_classes, num_views, batch_size, logits_all, preds_all, final_preds, Y, device, gamma=1.0):
    L_view = 0.0
    criterion = nn.CrossEntropyLoss()
    for v in range(num_views):
        L_view += criterion(logits_all[v], Y)
    L_view = L_view / num_views
    preds = final_preds
    L_vote = 0.0
    for i in range(num_views):
        for j in range(i + 1, num_views):
            agree = (preds[i] == preds[j]).float()
            L_vote += (1.0 - agree).mean()
    num_pairs = num_views * (num_views - 1) / 2
    if num_pairs > 0:
        L_vote = L_vote / num_pairs

    L_total = L_view + gamma * L_vote

    return L_total, L_view.item(), L_vote.item()
